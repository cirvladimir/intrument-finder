...
int main()
{
	...
	for (int instInd = 0; instInd < NUM_INSTS; instInd++)
	{
		...
		int numPts = r.read(insts[instInd], 5000, 15, 550, NUM_DATS, dat);
		totPts += numPts;
	...
	for (int contTestInd = 0;  contTestInd < 3; contTestInd++) 
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			r.normalize(sampData, r.read(insts[i], testContStarts[contTestInd], 10, 550, 1, sampData));
	...
	for (int testInd = 0; testInd < 4; testInd++)
	{
		...
		r.normalize(difSamp, r.read(tFile, testActStarts[testInd], 5, 550, 1, difSamp));
